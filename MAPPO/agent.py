	...
	def update_reward_model(self, sample):
		...
		if "AREL" in self.experiment_type:
			...
			if self.version == "agent_temporal_attn_weights":
				reward_loss = F.mse_loss(rewards.reshape(b, 1).sum(dim=-1), episodic_reward_batch.to(self.device)) + (self.classification_loss(action_prediction.reshape(-1, self.num_actions), actions_batch.long().permute(0, 2, 1).reshape(-1).to(self.device)) * agent_masks_batch.reshape(-1).to(self.device)).sum() / (agent_masks_batch.to(self.device).sum() + 1e-5) #+ 1e-4 * entropy_temporal_weights + 1e-4 * entropy_agent_weights
