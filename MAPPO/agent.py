...
class PPOAgent:
	...
	def update_reward_model(self, sample):
		...
		if "AREL" in self.experiment_type:
			...
			return reward_loss.item(), reward_prediction_loss.item(), inverse_dynamic_loss.item(), entropy_temporal_weights.item(), entropy_agent_weights.item(), grad_norm_value_reward.item()#, inverse_dynamic_loss.item(), grad_norm_inverse_dynamics.item()
		elif "STAS" in self.experiment_type:
