			...
			if "AREL" in self.experiment_type:
				...
				with torch.no_grad():
					...
					if self.experiment_type == "ATRR_temporal_attn_weights":
						...
						agent_temporal_contribution = F.softmax(torch.where(agent_masks_batch.bool(), (agent_weights.mean(dim=0).detach().cpu()*agent_temporal_contribution_weights).sum(dim=-2), -1e9), dim=1)
