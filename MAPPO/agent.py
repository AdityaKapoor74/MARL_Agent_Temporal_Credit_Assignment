...
class PPOAgent:
		...
		with torch.no_grad():
			if "AREL" in self.experiment_type:
				...
				with torch.no_grad():
					...
					temporal_scores, agent_scores, temporal_scores_final_temporal_block, state_prediction = self.reward_model(
					...
					if self.experiment_type == "ATRR_temporal_attn_weights":
						...
						agent_episodic_contribution = agent_weights.mean(dim=0).detach().cpu().sum(dim=-2).sum(dim=1) / agent_weights.mean(dim=0).detach().cpu().sum(dim=-2).sum(dim=1).sum(dim=-1, keepdims=True)
						...
						agent_temporal_contribution = torch.where(agent_masks_batch.bool(), (agent_weights.mean(dim=0).detach().cpu()).sum(dim=-2)*agent_temporal_contribution_weights, -1e9)
						agent_temporal_contribution = agent_temporal_contribution / agent_temporal_contribution.sum(dim=1, keepdims=True)
	...
	def update_reward_model(self, sample):
		...
		if "AREL" in self.experiment_type:
			...
			rewards, temporal_weights, agent_weights, temporal_weights_final_temporal_block, temporal_scores, agent_scores, temporal_scores_final_temporal_block, state_prediction = self.reward_model(
