			...
			if "AREL" in self.experiment_type:
				...
				with torch.no_grad():
					...
					if self.experiment_type == "ATRR_temporal_attn_weights":
						...
						agent_temporal_contribution_weights = torch.gather(temporal_weights.mean(dim=0).detach().cpu().reshape(b, n_a, t, t), 2, (team_mask_batch.sum(dim=-1)-1).reshape(b, 1, 1, 1).repeat(1, n_a, 1, t).long())
						agent_temporal_contribution = F.softmax(torch.where(agent_masks_batch.bool(), (agent_weights.mean(dim=0).detach().cpu()*agent_temporal_contribution_weights.transpose(1,2)).sum(dim=-2), -1e9), dim=1)
