...
class Time_Agent_Transformer(nn.Module):
	...
	def __init__(
		...
		self.ally_obs_compress_input = nn.Sequential(
			init_(nn.Linear(ally_obs_shape, self.comp_emb//2), activate=False),
			# nn.GELU(),
			)
		self.enemy_obs_compress_input = nn.Sequential(
			init_(nn.Linear(enemy_obs_shape, self.comp_emb//2), activate=False),
			# nn.GELU(),
			)
		...
		self.action_embedding = nn.Embedding(n_actions, self.comp_emb//2)

		self.return_embedding = init_(nn.Linear(1, self.comp_emb//2), activate=False)

		self.position_embedding = nn.Embedding(seq_length, self.comp_emb*2)
		...
		for i in range(depth):
			tblocks.append(
				TransformerBlock(emb=self.comp_emb*2, heads=heads, seq_length=seq_length, mask=True, dropout=dropout, wide=wide))
			if agent:
				tblocks.append(
					TransformerBlock_Agent(emb=self.comp_emb*2, heads=heads, seq_length=seq_length, n_agents=n_agents,
		...
		self.rblocks = nn.Sequential(
			...
			init_(nn.Linear(2*self.comp_emb*depth, self.comp_emb), activate=True),
			nn.GELU(),
			init_(nn.Linear(self.comp_emb, self.comp_emb), activate=True),
			nn.GELU(),
	...
	def forward(self, ally_obs, enemy_obs, actions, episodic_reward, team_masks=None, agent_masks=None):
		...
		enemy_obs = (self.enemy_obs_compress_input(enemy_obs)).sum(dim=1, keepdims=True)
		...
		position_embedding = self.position_embedding(torch.arange(self.seq_length).to(self.device))[None, None, :, :].expand(b, n_a, t, self.comp_emb*2)
		return_embedding = self.return_embedding(episodic_reward.reshape(b, 1)).reshape(b, 1, 1, self.comp_emb//2)
		...
		zeroth_state_action_embedding = torch.zeros((b, n_a, 1, self.comp_emb*2)).to(self.device)
		...
		if self.version == "temporal" or self.version == "temporal_v2":
			...
			indiv_agent_episode_len = (agent_masks.sum(dim=-2)-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, 2*self.comp_emb*self.depth).long() # subtracting 1 for indexing purposes
