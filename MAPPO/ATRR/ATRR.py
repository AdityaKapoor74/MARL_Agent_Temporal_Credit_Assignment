...
class Time_Agent_Transformer(nn.Module):
	...
	def forward(self, ally_obs, enemy_obs, actions, episodic_reward, team_masks=None, agent_masks=None, logprobs=None, train=False):
		...
		if self.version == "temporal" or self.version == "temporal_v2":
			...
			returns = self.return_mix_network(rewards, all_x, final_x.mean(dim=1).unsqueeze(1).repeat(1, t, 1), agent_masks).reshape(b, t, 1) #* importance_sampling.reshape(b, -1).prod(dim=-1).reshape(b, 1, 1) #* episodic_reward.to(self.device).reshape(b, 1, 1)
			...
			rewards_ = rewards.detach() * self.return_mix_network.w1.detach().reshape(b, t, n_a) * agent_masks.to(self.device) * importance_sampling #* importance_sampling.reshape(b, -1).prod(dim=-1).reshape(b, 1, 1) # * self.return_mix_network.b1.detach().reshape(b, t, 1) * episodic_reward.to(self.device).reshape(b, 1, 1)
