...
class TARR(nn.Module):
	...
	def forward(self, ally_states, enemy_states, states, actions, episode_length, episodic_reward, agent_temporal_mask):
		...
		next_state_embedding = torch.stack([state_embeddings[:, :, 1:, :], last_state_embedding], dim=-2)
		current_past_next_state_embeddings = torch.cat([state_embeddings, past_state_action_embeddings, next_state_embeddings], dim=-1)
