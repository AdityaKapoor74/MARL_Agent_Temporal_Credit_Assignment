...
class MAPPO:
    ...
    def __init__(self, env_fns, dictionary):
        ...
        if dictionary["device"] == "gpu":
            ...
            self.device = "cpu"
        self.envs = ShareSubprocVecEnv(env_fns, truncation_steps=dictionary["max_time_steps"])
        dummy_env = env_fns[0]()
        ...
        while self.num_episodes_done < self.max_episodes:
            ...
            if self.learn:
                ...
                self.agents.buffer.push(
                    ...
                    rewards_to_send, indiv_dones, self.worker_step_counter
                )
            ...
            states_actor, last_one_hot_actions, states_allies_critic, states_enemies_critic, mask_actions, indiv_dones = next_states_actor, one_hot_actions, next_states_allies_critic, next_states_enemies_critic, next_mask_actions, _combine_indiv_dones(info)
                ...
                if all(indiv_dones[worker_index]) or self.worker_step_counter[worker_index] == self.max_time_steps:
                    ...
                    print(
                        "EPISODE: {} | REWARD: {} | TIME TAKEN: {} / {} | Num Allies Alive: {} | Num Enemies Alive: {} \n".format(
                            ...
                            step,
                            self.max_time_steps,
                            info["last_info"]["num_allies"][worker_index],
                            info["last_info"]["num_enemies"][worker_index],
                        )
                    )
                    
                    print("INDIV REWARD STREAMS", episode_indiv_rewards[worker_index], "AGENTS DEAD", info["last_info"]["indiv_dones"][worker_index])
                    print("*"*100)
                    ...
                    if self.learn:
                        ...
                        last_states_actor = np.array(info["last_obs"][worker_index])
                        last_states_actor = np.expand_dims(np.concatenate((self.agent_ids[worker_index], last_states_actor), axis=-1), axis=0)
                        last_states_allies_critic = np.expand_dims(np.concatenate((self.agent_ids[worker_index], info["last_info"]["ally_states"][worker_index]), axis=-1), axis=0)
                        last_states_enemies_critic = np.expand_dims(np.concatenate((self.enemy_ids[worker_index], info["last_info"]["enemy_states"][worker_index]), axis=-1), axis=0)
                        last_mask_actions = np.expand_dims(np.array(info["last_info"]["avail_actions"][worker_index], dtype=int), axis=0)
                        last_indiv_dones = np.expand_dims(info["last_info"]["indiv_dones"][worker_index], axis=0)
                    ...
                    last_one_hot_actions[worker_index] = np.zeros((self.num_agents, self.num_actions))
                    ...
                    rnn_hidden_state_q[worker_index] = np.zeros((self.rnn_num_layers_q, self.num_agents, self.rnn_hidden_q))
...
if __name__ == '__main__':
    ...
    for i in range(1,6):
        ...
        env_name = "5m_vs_6m"
        experiment_type = "temporal_team" # episodic_team, episodic_agent, temporal_team, temporal_agent, AREL, SeqModel, RUDDER
