...
class MAPPO:
	...
	def run(self):  
		...
		for episode in range(1,self.max_episodes+1, self.ppo_eps_elapse_update_freq):
	...
	# 		global_obs = None
	# 	elif "GFootball" in self.environment:
	# 		global_obs = local_obs = self.env[process_id].reset()
	# 		global_obs = np.array(global_obs)
	# 		mask_actions = np.ones([self.num_agents, self.num_actions])
	# 		ally_states, enemy_states = None, None
	# 	else:
	# 		local_obs = self.env[process_id].reset()
	# 		global_obs = self.env[process_id].get_state()
	# 		mask_actions = np.ones([self.num_agents, self.num_actions])
	# 		ally_states, enemy_states = None, None
	...
	# 		print("process id", process_id, "step", step)

	# 		if self.gif:
	# 			# At each step, append an image to list
	# 			# import time
	# 			# time.sleep(0.1)
	# 			self.env[process_id].render()
	# 			# Advance a step and render a new image
	# 			with torch.no_grad():
	...
	# 		elif "Alice_and_Bob" in self.environment:
	# 			next_global_obs = self.env[process_id].get_state()
	...
	# 		queue.put((
	# 			process_id, ally_states, enemy_states, value, rnn_hidden_state_v, \
	# 			global_obs, local_obs, rnn_hidden_state_actor, action_logprob, actions, one_hot_actions, mask_actions, \
	# 			rewards_to_send, indiv_dones, dones
	# 			))

				
	# 		ally_states, enemy_states, global_obs, local_obs, last_actions, mask_actions, indiv_dones, dones = next_ally_states, next_enemy_states, next_global_obs, next_local_obs, actions, next_mask_actions, next_indiv_dones, next_dones
	# 		rnn_hidden_state_v, rnn_hidden_state_actor = next_rnn_hidden_state_v, next_rnn_hidden_state_actor
	...
	# 			if "StarCraft" in self.environment:
	# 				queue.put((process_id, episode_reward, episode_indiv_rewards, info["num_allies"], info["num_enemies"], info["all_enemies_dead"], info["all_allies_dead"], final_timestep, value, indiv_dones, dones))
	# 			elif self.environment in ["Alice_and_Bob", "GFootball"]:
	# 				queue.put((process_id, episode_reward, episode_indiv_rewards, final_timestep, value, indiv_dones, dones))
	...
	# 	# run multiple episodes with step count as num_threads
	# 	for episode in range(1, self.max_episodes+1, self.ppo_eps_elapse_update_freq):

	# 		processes = []
	# 		queue = mp.Queue()
	# 		num_processes = min(self.ppo_eps_elapse_update_freq, self.agents.buffer.num_episodes-self.agents.buffer.episode_num)
	# 		collect_data = [[] for i in range(num_processes)]
	# 		done_dict = {}
	...
	# 				done_dict[transition[0]] = all(transition[-2])
	# 				collect_data[transition[0]].append(transition)
	...
	# 		# store all the collected data in the agent buffer
	# 		for process_id in range(num_processes):
	# 			for data in collect_data[process_id][:-1]: # last data point is to be stored differently
					
	# 				_, ally_states, enemy_states, value, rnn_hidden_state_v, \
	# 				global_obs, local_obs, rnn_hidden_state_actor, action_logprob, actions, one_hot_actions, mask_actions, \
	# 				rewards_to_send, indiv_dones, dones = data
	...
	# 			print(len(collect_data[process_id][-1]))

	# 			if "StarCraft" in self.environment:
	# 				_, episode_reward, episode_indiv_rewards, num_allies, num_enemies, all_enemies_dead, all_allies_dead, final_timestep, value, indiv_dones, dones = collect_data[process_id][-1]
	# 			elif self.environment in ["Alice_and_Bob", "GFootball"]:
	# 				_, episode_reward, episode_indiv_rewards, final_timestep, value, indiv_dones, dones = collect_data[process_id][-1]
	...
	# 			print("*"*100)
	# 			print("EPISODE: {} | REWARD: {} | TIME TAKEN: {} / {} | INDIV REWARD STREAMS: {} \n".format(episode+process_id, np.round(episode_reward, decimals=4), final_timestep, self.max_time_steps, episode_indiv_rewards))
	# 			if "StarCraft" in self.environment:
	# 				print("Num Allies Alive: {} | Num Enemies Alive: {} | AGENTS DEAD: {} \n".format(num_allies, num_enemies, indiv_dones))
	# 			elif "Alice_and_Bob" in self.environment:
	# 				print("AGENTS DONE: {} \n".format(indiv_dones))
	# 			print("*"*100)

	# 			if self.save_comet_ml_plot:
	# 				self.comet_ml.log_metric('Episode_Length', final_timestep, episode+process_id)
	# 				self.comet_ml.log_metric('Reward', episode_reward, episode+process_id)
	# 				if "StarCraft" in self.environment:
	# 					self.comet_ml.log_metric('Num Enemies', num_enemies, episode+process_id)
	# 					self.comet_ml.log_metric('Num Allies', num_allies, episode+process_id)
	# 					self.comet_ml.log_metric('All Enemies Dead', all_enemies_dead, episode+process_id)
	# 					self.comet_ml.log_metric('All Allies Dead', all_allies_dead, episode+process_id)
	# 				elif self.environment in ["Alice_and_Bob", "GFootball"]:
	# 					self.comet_ml.log_metric('Agents Done', dones, episode+process_id)
	...
	# 			if episode+process_id > self.save_model_checkpoint and self.eval_policy:
	# 				self.rewards_mean_per_1000_eps.append(sum(self.rewards[episode+process_id-self.save_model_checkpoint:episode+process_id])/self.save_model_checkpoint)
	# 				self.timesteps_mean_per_1000_eps.append(sum(self.timesteps[episode+process_id-self.save_model_checkpoint:episode+process_id])/self.save_model_checkpoint)

				
	# 			if not((episode+process_id)%self.save_model_checkpoint) and episode+process_id!=0 and self.save_model:	
	# 				torch.save(self.agents.critic_network_v.state_dict(), self.critic_model_path+'_V_epsiode'+str(episode+process_id)+'.pt')
	# 				torch.save(self.agents.policy_network.state_dict(), self.actor_model_path+'_epsiode'+str(episode+process_id)+'.pt')  

	# 			if self.learn and not((episode+process_id)%self.ppo_eps_elapse_update_freq) and episode+process_id != 0:
	# 				if self.experiment_type == "uniform_team_redistribution":
	# 					b, t, n_a = self.agents.buffer.rewards.shape
	# 					episodic_avg_reward = np.sum(self.agents.buffer.rewards[:, :, 0], axis=1)/self.agents.buffer.episode_length
	# 					self.agents.buffer.rewards[:, :, :] = np.repeat(np.expand_dims(np.repeat(np.expand_dims(episodic_avg_reward, axis=-1), repeats=t, axis=-1), axis=-1), repeats=n_a, axis=-1)
	# 					self.agents.buffer.rewards *= (1-self.agents.buffer.indiv_dones[:, :-1, :])
	# 					self.agents.update(episode+process_id)
	# 				elif self.use_reward_model and episode+process_id > self.warm_up_period:
	# 					# finetune
	# 					# sample = self.agents.buffer.reward_model_obs, self.agents.buffer.actions, self.agents.buffer.one_hot_actions, self.agents.buffer.rewards[:, :, 0], 1-self.agents.buffer.team_dones[:, :-1], 1-self.agents.buffer.agent_dones[:, :-1, :], self.agents.buffer.episode_length
	# 					# self.agents.update_reward_model(sample)
	...
	# 			if self.learn:
	# 				if self.use_reward_model and self.reward_batch_size <= self.agents.reward_model_buffer.length and episode+process_id != 0 and (episode+process_id) % self.update_reward_model_freq == 0:
	# 					reward_loss_batch, grad_norm_reward_batch = 0.0, 0.0
	# 					if "AREL" in self.experiment_type:
	# 						reward_var_batch = 0.0
	# 					elif "TAR^2" in self.experiment_type:
	...
	# 					if self.comet_ml is not None:
	# 						self.comet_ml.log_metric('Reward_Loss', reward_loss_batch, episode+process_id)
	# 						self.comet_ml.log_metric('Reward_Grad_Norm', grad_norm_reward_batch, episode+process_id)

	# 						if "AREL" in self.experiment_type:
	# 							self.comet_ml.log_metric('Reward_Var', reward_var_batch, episode+process_id)
	# 						elif "TAR^2" in self.experiment_type:
	# 							self.comet_ml.log_metric('Entropy_Temporal_Weights', entropy_temporal_weights_batch, episode+process_id)
	# 							self.comet_ml.log_metric('Entropy_Agent_Weights', entropy_agent_weights_batch, episode+process_id)

	# 							self.comet_ml.log_metric('Reward Prediction Loss', reward_prediction_loss_batch, episode+process_id)
	# 							self.comet_ml.log_metric('Reward Dynamic Loss', dynamic_loss_batch, episode+process_id)
							
	# 			if self.eval_policy and not((episode+process_id)%self.save_model_checkpoint) and episode+process_id!=0:
	# 				np.save(os.path.join(self.policy_eval_dir,self.test_num+"reward_list"), np.array(self.rewards), allow_pickle=True, fix_imports=True)
	# 				np.save(os.path.join(self.policy_eval_dir,self.test_num+"mean_rewards_per_1000_eps"), np.array(self.rewards_mean_per_1000_eps), allow_pickle=True, fix_imports=True)
	# 				np.save(os.path.join(self.policy_eval_dir,self.test_num+"timestep_list"), np.array(self.timesteps), allow_pickle=True, fix_imports=True)
	# 				np.save(os.path.join(self.policy_eval_dir,self.test_num+"mean_timestep_per_1000_eps"), np.array(self.timesteps_mean_per_1000_eps), allow_pickle=True, fix_imports=True)
...
if __name__ == '__main__':
	...
	for i in range(1, 6):
		...
		if "StarCraft" in dictionary["environment"]:
			...
			env = [FootballEnv()]*dictionary["ppo_eps_elapse_update_freq"]
			...
			if dictionary["ppo_eps_elapse_update_freq"] > 1:
