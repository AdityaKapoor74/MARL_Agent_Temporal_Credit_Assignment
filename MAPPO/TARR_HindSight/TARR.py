...
class ImportanceSamplingHyperNetwork(nn.Module):
	...
	def forward(self, importance_sampling_ratio, all_agent_state_action, final_state_action_embedding, agent_masks):
		...
		w1 = self.hyper_w1(torch.cat([all_agent_state_action, final_state_action_embedding.mean(dim=1, keepdim=True).unsqueeze(1).repeat(1, n_a, t, 1)], dim=-1).transpose(1, 2).reshape(-1, e*2))
...
class Time_Agent_Transformer(nn.Module):
	...
	def __init__(
		...
		self.dynamics_model = nn.Sequential(
			init_(nn.Linear(self.comp_emb*(2*depth+1), self.comp_emb), activate=True),
			nn.GELU(),
	...
	def forward(self, ally_obs, enemy_obs, obs, actions, episodic_reward, agent_masks):
		...
		x_goal_states = (all_x*agent_masks.transpose(1, 2).unsqueeze(-1)).unsqueeze(-3).repeat(1, 1, t, 1, 1)
		state_past_state_action_embeddings = (state_past_state_action_embeddings*agent_masks.transpose(1, 2).unsqueeze(-1)).unsqueeze(-2).repeat(1, 1, 1, t, 1)
		current_context_goal = torch.cat([state_past_state_action_embeddings, x_goal_states], dim=-1) * upper_triangular_mask # b, n_a, t, t, -1
		action_prediction = self.dynamics_model(current_context_goal.detach())
		...
		rewards = F.relu(self.rblocks(all_x).view(b, n_a, t).contiguous().transpose(1, 2) * agent_masks.to(self.device) * torch.sign(episodic_reward.to(self.device).reshape(b, 1, 1))) * torch.sign(episodic_reward.to(self.device).reshape(b, 1, 1))
